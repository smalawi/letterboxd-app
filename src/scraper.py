...
def get_all_user_film_data(user):
	"""
	...
	for film_url in get_watched_film_urls(user):
		film_url_text = requests.get(film_url).text
		...
		for film_tag in soup.find_all('li', class_='poster-container'):
			...
			film_data = get_film_data(film_url)
...
def get_all_user_review_data(user):
	"""
	...
	for reviews_url in get_review_urls(user):
